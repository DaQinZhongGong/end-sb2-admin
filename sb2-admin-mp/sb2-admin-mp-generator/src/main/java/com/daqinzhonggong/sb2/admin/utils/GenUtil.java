...
import static com.daqinzhonggong.sb2.admin.utils.FileUtil.SYS_TEM_DIR;
...
public class GenUtil {
  ...
  public static String download(List<ColumnInfo> columns, GenConfig genConfig) throws IOException {
    // 拼接的路径：/tmp-gen-temp/，这个路径在Linux下需要root用户才有权限创建,非root用户会权限错误而失败，更改为： /tmp/gen-temp/
    // String tempPath =SYS_TEM_DIR + "gen-temp" + File.separator + genConfig.getTableName() + File.separator;
    String tempPath = SYS_TEM_DIR + "gen-temp" + File.separator + genConfig.getTableName()
        + File.separator;
    Map<String, Object> genMap = getGenMap(columns, genConfig);
    TemplateEngine engine = TemplateUtil.createEngine(
        new TemplateConfig("template", TemplateConfig.ResourceMode.CLASSPATH));
    ...
    for (String templateName : templates) {
      ...
      String filePath = getAdminFilePath(templateName, genConfig,
          genMap.get("className").toString(), tempPath + "admin" + File.separator);
    ...
    for (String templateName : templates) {
      ...
      String srcPath =
          path + "src" + File.separator + "views" + File.separator + genMap.get("changeClassName")
  ...
  public static void generatorCode(List<ColumnInfo> columnInfos, GenConfig genConfig)
    ...
    for (String templateName : templates) {
      ...
      String filePath = getAdminFilePath(templateName, genConfig,
          genMap.get("className").toString(), rootPath);
  ...
  private static String getAdminFilePath(String templateName, GenConfig genConfig, String className,
    ...
    if ("QueryCriteria".equals(templateName)) {
      return packagePath + "domain" + File.separator + "vo" + File.separator + className
          + "QueryCriteria.java";
    }
